...
int main(int argc, char *argv[])
{
    ...
    time_t cap_sec = 0;   /* prevent multiple fcking prints */
...
if (pcap_setfilter(handle, &fp) == -1) {
	...
	while(1) {
		u_char* current_pos;
		if(pcap_next_ex(handle, &header,&packet)) continue;
		current_pos = (u_char*)packet;
		/* Print its length */
		//printf("Jacked a packet with length of [%d] at %d\n", header->caplen,(int)header->ts.tv_sec);
		//sleep(1);
		if(header->caplen < 1 || header->ts.tv_sec == cap_sec && header->ts.tv_usec == cap_usec) continue;
